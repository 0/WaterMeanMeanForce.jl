# Constrained OBABO integrator.

struct ConstrainedOBABO{P,D,A,N} <: Integrator
...
function ConstrainedOBABO(
    ...
    ConstrainedOBABO{P,D,A,N}(beta, nmt, nms, oscprop, cstr, forceapp,
                              thermostat, NaN)
end

is_constrained(::Type{ConstrainedOBABO}) = true

function N_dof(::ConstrainedOBABO{P,D,A,N}) where {P,D,A,N}
    N * A * D * P - 1
end

thermostat_enabled(int::ConstrainedOBABO) = !isnothing(int.thermostat)

function step!(s::State{P,D,A,N},
               int::ConstrainedOBABO{P,D,A,N}) where {P,D,A,N}
...
function energy(int::ConstrainedOBABO{P,D,A,N},
...
function get_force(int::ConstrainedOBABO)
    int.forceapp.current_force.initialized || error("Force not initialized.")

    int.forceapp.current_force.f
end

get_temperature(int::ConstrainedOBABO) = int.temperature[]


registered_integrators["c-OBABO"] = ConstrainedOBABO
